function [schm,schm_tp] = getDPHtransSchemesWithNfreePrm(nfp,sumexp,Dmin,Dmax,...
    schm_tp)
% [schm,schm_tp] = getDPHtransSchemesWithNfreePrm(nfp,sumexp,Dmax,schm_tp)
%
...
%
% nfp: number of free DPH parameters
% sumexp: 1 to return transition schemes only for hyperexponential DPH, 0
%  otherwise
...
% 
% example:
% >> schm = getTransSchemesWithNfreePrm(4,false,2)

% initialize output
schm = [];

% return hyperexponential scheme
if sumexp
    D = (nfp+1)/2;
    if round(D)~=D || D>Dmax
        return
    ...
    schm = zeros(D+2,D+2);
    schm(1,2:end-1) = 1; % initial state probabilities
    schm(2:end-1,end) = 1; % exit probabilities
    schm = {schm};
    return
end
...
Dmax = min([nfp,Dmax]);
Dmin = max([Dmin,ceil((sqrt(4*(nfp+1))-1)/2)]);
if Dmax<Dmin
    schm = Inf;
    return
end
...
for D = Dmin:Dmax
    [schm_D,schm_tp] = fillDPHtransSchemeWithNprob(D,nfp+1,schm_tp);
    if isempty(schm_D)
        continue
    end
...
function [schm,schm_tp] = fillDPHtransSchemeWithNprob(D,n,schm_tp)
% [schm,schm_tp] = fillDPHtransMatWithNtrans(D,n,schm_tp)
% 
% Return all valid transition schemes for D degenerate states and n free 
% probabilities
%
% D: state degeneracy
% n: number of free probabilities (initial, transition and exit probabilities)
...
n_tp_0 = numel(schm_tp{D});
if n_tp_0<(n_tp_max+1)
    schm_tp{D} = [schm_tp{D},cell(1,n_tp_max+1-n_tp_0)];
end
...
for n_ip = 1:D 
    ...
    for c_ip = 1:size(ip,1)
        ...
        for n_ep = 1:D 
            n_tp = n-n_ip-n_ep; % state transition probabilities
            if n_tp<0 || n_tp>(D*(D-1))
                continue
            end
            ...
            if isempty(schm_tp{D}{id})
                schm_tp{D}{id} = getTransSchemesForNtrans(D,n_tp);
            end
